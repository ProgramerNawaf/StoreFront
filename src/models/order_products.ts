...
export class Order_Products_Store {
  ...
  async checkoutOrder(customer_id: Number, order_id: Number): Promise<void> {
    ...
    const conn = await client.connect();
    console.log('33');
    let sql = 'SELECT * FROM order_products WHERE id = $1';
    ...
    const result1 = await conn.query(sql, [customer_id]);
    ...
    const result2 = await conn.query(sql, [order.product_id]);
    ...
    sql = 'UPDATE customer SET balance =$1 WHERE customer_id = $2';
    const result3 = await conn.query(sql, [newBalance, customer_id]);
    console.log('77' + product.merchant_id);
    sql = 'SELECT * FROM merchant WHERE merchant_id = $1';
    const result4 = await conn.query(sql, [product.merchant_id]);
    ...
    sql = 'UPDATE merchant SET revenue =$1 WHERE merchant_id = $2';
    const result5 = await conn.query(sql, [newRevenue, merchant.merchant_id]);
    console.log('99' + merchant.merchant_id);

    sql = 'UPDATE order_products SET status =$1 WHERE id = $2';
    const result6 = await conn.query(sql, ['DONE', order.id]);
    conn.release();
  }
}
